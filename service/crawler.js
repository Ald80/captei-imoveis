...
async function extractDetails(link) {
  try {
    ...
    const descricao = $linkPage(
      'div.flex.flex-col.gap-2 > p.text-md.text-foxter-brand-900.leading-8',
    )
      .eq(3)
      .text()
      .trim();
    const codes = $linkPage('p:contains("Código do anunciante")')
      .first()
      .text()
      .trim();
    const [, code123iText] = codes.split('|').map((code) => code.trim());
    const code123i = code123iText.match(/Código 123i: (\w+)/)[1];
    const titulo = $linkPage(
      'h1.text-lg.md\\:xl.lg\\:text-2xl.text-foxter-brand-900.font-bold.mb-2.2xl\\:mb-3',
    ).text();
    const detailsContainer = $linkPage(
      'div.flex.flex-wrap.py-3.items-start.justify-evenly.md\\:justify-between.text-center',
    );
    const quartos = detailsContainer
      .find('p:contains("dorms")')
      .first()
      .text()
      .trim();
    const banheiros = detailsContainer
      .find('p:contains("banheiros")')
      .first()
      .text()
      .trim();
    const garagem = detailsContainer
      .find('p:contains("vagas")')
      .first()
      .text()
      .trim();
    const areaUtil = detailsContainer
      .find('p:contains("privativos")')
      .first()
      .text()
      .trim();
  ...
  try {
    ...
    for (let i = 0; i < divs.length; i++) {
      ...
      if (link) {
        ...
        const endereco = section
          .find('div.flex.text-sm.justify-start.text-foxter-brand-900')
          .last()
          .text()
          .trim();
...
export async function fetchMultiplePagesImoveis(
  base_url,
  portalNome,
  numeroDePaginas,
) {
